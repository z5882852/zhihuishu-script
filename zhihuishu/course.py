...
class ShareCourse:
    ...
    def query_course_info(self, recruit_and_course_id):
        ...
        response = self.session.post(url, headers=self.headers, data=data)
        if response.status_code != 200:
            raise Exception(f"查询课程信息失败，status_code: {response.status_code}, response: {response.text}")
        data = response.json()
        if data.get("code", -1) != 0:
            raise Exception(f"查询课程信息失败，response_code: {data.get('code', -1)}, response: {data}")
